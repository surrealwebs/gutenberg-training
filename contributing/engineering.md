...
```bash
git clone git@github.com:surrealwebs/gutenberg-training.git gutenberg-training
cd gutenberg-training
```
...
Run npm run dev to build the latest version of the Gutenberg Training plugin,
...
```bash
wp plugin activate gutenberg-training
```

If running this from the included Docker environment:

```bash
npm run wp -- wp plugin activate gutenberg-training
```
...
This will create an `gutenberg-training.zip` in the plugin directory which you can install. The contents of this ZIP are also located in the `build` directory which you can `rsync` somewhere as well if needed.

## Creating a pre-release

1. Create changelog draft on [Wiki page](https://github.com/surrealwebs/gutenberg-training/wiki/Release-Changelog-Draft).
1. Check out the branch intended for release (`develop` for major, `x.y` for minor) and pull latest commits.
1. Bump plugin versions in `gutenberg-training.php`.
1. Do `npm install`.
1. Do `npm run build` and install the `gutenberg-training.zip` onto a normal WordPress install running a stable release build; do smoke test to ensure it works.
1. [Draft new release](https://github.com/surrealwebs/gutenberg-training/releases/new) on GitHub targeting the required branch (`develop` for major, `x.y` for minor).
    ...
    1. Attach the `gutenberg-training.zip` build to the release.
...
1. Check out the branch intended for release (`develop` for major, `x.y` for minor) and pull latest commits.
1. Do `npm install`.
1. Bump plugin versions in `gutenberg-training.php`. Ensure patch version number is supplied for major releases, so `1.2-RC1` should bump to `1.2.0`.
1. Ensure "Tested Up To" is updated to current WordPress version.
1. Do `npm run build` and install the `gutenberg-training.zip` onto a normal WordPress install running a stable release build; do smoke test to ensure it works.
1. Optionally do sanity check by comparing the `build` directory with the previously-deployed plugin on WordPress.org for example: `svn export https://plugins.svn.wordpress.org/gutenberg-training/trunk /tmp/gutenberg-training-trunk; diff /tmp/gutenberg-training-trunk/ ./build/` (instead of straight `diff`, it's best to use a GUI like `idea diff`, `phpstorm diff`, or `opendiff`).
1. [Draft new release](https://github.com/surrealwebs/gutenberg-training/releases/new) on GitHub targeting the required branch (`develop` for major, `x.y` for minor):
